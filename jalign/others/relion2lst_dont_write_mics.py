...
def main():
    ...
    usage = """prog [options] <jalign STAR file>
...
"""
    ...
    parser.add_argument('--allrelion',type=int, default=0, help='convert all jalign parameters to lst file if set to 1')
    parser.add_argument("--verbose", "-v", dest="verbose", action="store", metavar="n", type=int, default=0, help="verbose level [0-9], higner number means higher level of verboseness")
    parser.add_argument("--dir", type=str, default='', help='mkdir dir')
...
#	try: os.mkdir("particles")
    ...
    params = []
    for i in xrange(len(star["rlnMicrographName"])):
        name=star["rlnMicrographName"][i]
        #name=os.path.join(topdir, name)
        #imgnum=int(star["rlnImageName"][i].split("@")[0])-1

        if (star.has_key("rlnMagnificationCorrection")): #for old version jalign star filr
    ...
    #		microname="particles/{}_{:04d}.hdf".format(base_name(name),micronum)
            microname=(directory+"/{}_{:04d}.hdf").format(base_name(name),micronum)
        ...
        param = [fnum, name, defocus, dfdiff, dfang]
    ...
    fp.write("#LST\n")
    for p in params:
        fp.write("%d\t%s\tdefocus=%s\tdfdiff=%s\tdfang=%s" % (p[0], p[1], p[2], p[3], p[4]))
        if options.allrelion:
            vars = sorted(star.keys())
            for vi, var in enumerate(vars):
                fp.write("\t%s=%s" % (var.split("rln")[-1], p[5+vi]))
        fp.write("\n")
    fp.close()
