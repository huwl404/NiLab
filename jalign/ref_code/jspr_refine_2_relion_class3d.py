...
def main():
    global df_index, dfdiff_index, dfang_index, euler_index, center_index
    ...
    for i in range(meta, len(lst_line)):
        ...
        r.write("%06d" % int(num))
        r.write("@" + filename + "\t")
        r.write(str(dfu) + "\t" + str(dfv) + "\t" + str(dfang) + "\t300.000000\t2.7000000\t0.100000\t" + str(
            apix_c) + "\t5.000000\t")
        r.write("0.14000\t1\t")
        r.write(str(math.fmod(euler2 - 90, 360)) + "\t" + str(euler1) + "\t" + str(math.fmod(euler3 + 90, 360)) + "\t")
        # only for EMAN ICOS to jalign I3 symmetry
