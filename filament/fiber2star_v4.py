...
# -*- coding: utf-8 -*-
"""
Fiber tracking in cryo-ET tomograms using Hessian-based vesselness detection,
skeletonization, and directional segment merging.

Designed for tracking dark hollow tubes (e.g., microtubule doublets) in
cryo-electron tomograms. Does NOT use template matching / NCC, which is
unreliable for doublet structures (partial matching → 3 false lines,
missing-wedge Z-stretching, near-zero NCC contrast).

Pipeline:
  1. Preprocess: bin, normalize, Butterworth low-pass
  2. Membrane detection → non-membrane mask (exclude membrane regions)
  3. Sato vesselness (multi-scale, dark ridges) within non-membrane region
  4. Binary mask extraction: threshold → opening → remove small CCs
  5. Reference direction: extract mask at high percentile → PCA dominant
     direction from thick CCs (auto-lower percentile until >= 3 CCs)
  6. Skeleton junction filter:
     a. Skeletonize base mask → bridge micro-gaps → re-skeletonize
     b. Detect branch points (degree ≥ 3 in 26-connectivity)
     c. Remove branch-point neighborhoods → sub-branches
     d. PCA direction per branch → keep aligned with dominant direction
     e. Re-bridge kept branches → PCA-ordered segments
  7. Segment merging: proximity + directional alignment + extension
     (inspired by Struwwel Tracer CFS fusion, Sazzed et al. IJMS 2023)
...
def sato_vesselness(
    ...
    """
    Multi-scale Sato tubularity enhancement for dark tubes (black_ridges=True).
    sigma in pixels. Returns vesselness in [0,1].
    """
    v = np.asarray(vol_zyx, dtype=np.float32)
    sigma_min = max(float(sigma_min), 0.5)
    sigma_max = max(float(sigma_max), sigma_min)
    sigma_steps = max(int(sigma_steps), 1)
    sigmas = np.unique(
        np.linspace(sigma_min, sigma_max, sigma_steps, dtype=np.float32)
    ).tolist()
    if not sigmas:
        sigmas = [sigma_min]
...
def extract_binary_mask(
    ...
    """
    Extract cleaned binary mask from vesselness volume (NO skeletonization).

    Steps:
      1. Mask vesselness with non-membrane region
      2. Threshold at percentile
      3. Binary opening to disconnect noise bridges
    ...
    if finite_vals.size == 0:
        print("  vesselness is all zero after membrane masking")
        return None
    th = float(cp.percentile(finite_vals, vesselness_percentile))
    mask_gpu = v_gpu >= th
    if not bool(mask_gpu.sum().get()):
        print("  mask is empty after thresholding")
        return None
    if debug_prefix is not None:
        print(f"  {vesselness_percentile}% percentile => threshold = {th:.6f}")
        save_mrc(debug_prefix.with_name(debug_prefix.name + "_5_vess_binary.mrc"), cp.asnumpy(mask_gpu.astype(cp.float32)), voxel_size=voxel_size)

    if opening_radius > 0:
        selem_open = cskimage.morphology.ball(int(opening_radius))
        mask_gpu = cskimage.morphology.binary_opening(mask_gpu, selem_open)
        if not bool(mask_gpu.any().get()):
            print("  mask empty after binary opening")
            return None
    if debug_prefix is not None:
        print(f"  binary opening: radius = {opening_radius}")
        save_mrc(debug_prefix.with_name(debug_prefix.name + "_6_vess_opening.mrc"), cp.asnumpy(mask_gpu.astype(cp.float32)), voxel_size=voxel_size)
    ...
    if debug_prefix is not None:
        ...
        save_mrc(debug_prefix.with_name(debug_prefix.name + "_7_vess_clean.mrc"), cp.asnumpy(mask_gpu.astype(cp.float32)), voxel_size=voxel_size)
...
def _process_single_tomogram(args: argparse.Namespace, tomo_path: Path, out_prefix: Path) -> None:
    ...
    if bin_factor > 1:
        ...
        print(f"  binned shape: {vol.shape}, effective voxel: {voxel:.3f} A")
        if args.debug:
    ...
    print(f"detecting membrane: sheetness_percentile = {args.sheetness_percentile}%")
    membrane_mask = detect_membrane_sheetness(filtered, sheetness_percentile=float(args.sheetness_percentile))
    ...
    if args.debug:
        save_mrc(out_prefix.with_name(out_prefix.name + "_3_membrane_mask.mrc"), membrane_mask, voxel_size=voxel)
        save_mrc(out_prefix.with_name(out_prefix.name + "_3_nonmembrane_mask.mrc"), nonmembrane_mask, voxel_size=voxel)

    # ---- Sato vesselness ----
    print(f"computing Sato vesselness: sigma = [{args.sigma_min}, {args.sigma_max}], {args.sigma_steps} scales, dark tubes")
    vess = sato_vesselness(filtered, sigma_min=args.sigma_min, sigma_max=args.sigma_max, sigma_steps=args.sigma_steps)
    if args.debug:
        save_mrc(out_prefix.with_name(out_prefix.name + "_4_sato.mrc"), vess, voxel_size=voxel)

    # ---- Extract binary mask at base percentile ----
    print("extracting binary mask")
    ...
    ref_pct = float(args.ref_vesselness_percentile)
        ...
        if ref_mask is not None:
            ...
            if n_ccs >= 3:
                ...
                if dominant_dir is not None:
                    ...
                    if args.debug:
                        save_mrc(out_prefix.with_name(out_prefix.name + "_7b_ref_mask.mrc"), ref_mask.astype(np.float32), voxel_size=voxel)
                    break
        ref_pct -= 2.0
